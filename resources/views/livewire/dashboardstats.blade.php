...
new class extends Component {
    ...
    {
        return [
            'notesSendCount' => Auth::user()->notes()->where('send_date', '<', now())->where('is_published', true)->count(),
...
<div>
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-2">
        <div class="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
            ...
            <div class="mt-6">
                <p class="text-3xl font-bold leading-9 text-gray-900 dark:text-gray-100">{{ $notesSendCount }}</p>
            </div>
        </div>
        <div class="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
            <div class="flex items-center">
